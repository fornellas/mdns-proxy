...
case "$(uname -s)" in
	...
	Darwin)
		...
		DOCKER_USER=mdnsproxy
		DOCKER_GROUP=mdnsproxy
...
mkdir -p "${XDG_CACHE_HOME}/mdns-proxy"
...
docker run \
	...
	${TTY} \
	--interactive \
	--volume "${GIT_ROOT}:${DOCKER_HOME}/mdns-proxy" \
	--volume "${XDG_CACHE_HOME}/mdns-proxy:${DOCKER_XDG_CACHE_HOME}/mdns-proxy" \
	--env "XDG_CACHE_HOME=${DOCKER_XDG_CACHE_HOME}" \
	${GO_ENV_ARGS} \
	--workdir ${DOCKER_HOME}/mdns-proxy \
	${DOCKER_IMAGE} \
